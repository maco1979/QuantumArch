...
from quantum_core import (
    QuantumArch, QuantumSuperpositionAttention, QuantumEntanglementLayer,
    QuantumCollapseInference, QGD, CayleyLinear, ComplexLayerNorm,
    ModReLU, complex_to_polar, von_neumann_entropy, born_normalize,
...
qsa = QuantumSuperpositionAttention(dim=64, num_heads=4, head_dim=16, topk_ratio=0.2, mode='topk').to(device)
...
print(f"  Input: {tuple(x_in.shape)}  Output: {tuple(out_e.shape)}  Time: {(time.time()-t0)*1000:.1f}ms")
...
print(f"  Input: {tuple(out_e.shape)}  Output: {tuple(out_c.shape)}  Time: {(time.time()-t0)*1000:.1f}ms")
print(f"  entropy={mc['collapse_entropy']:.4f}  early_exit_rate={mc['collapse_early_exit_rate']:.2%}")
...
print(f"  Input/Output: {tuple(xa.shape)}  Nonlinearity std={nl:.4f}  {'OK' if nl > 0.01 else 'FAIL'}")
...
model = QuantumArch(
    ...
    ffn_dim=128, max_seq_len=512, topk_ratio=0.2,
    collapse_enabled=True, tau_low=0.5, tau_high=1.5,
    dropout=0.0, qsa_mode='topk', output_dim=1000,
).to(device)
...
for k, lm in res['layer_metrics'].items():
    print(f"    {k}: attn_ent={lm.get('attention_entropy','N/A'):.4f}" if isinstance(lm.get('attention_entropy'), float) else f"    {k}: {lm}")
...
for step in range(5):
    ...
    loss = nn.functional.cross_entropy(r['output'].view(-1, 1000), torch.randint(0, 1000, (16,), device=device))
