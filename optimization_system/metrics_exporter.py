...
class QuantumArchMetrics:
    ...
    def __init__(self, port: int = 8000):
        ...
        # 1. 训练指标
        self.training_loss = Gauge("quantum_arch_training_loss", "训练损失", registry=self.registry)
        self.validation_loss = Gauge(
            "quantum_arch_validation_loss", "验证损失", registry=self.registry
        )
        self.accuracy = Gauge("quantum_arch_accuracy", "准确率", registry=self.registry)

        # 2. QSA指标
        self.qsa_topk_ratio = Gauge(
            "quantum_arch_qsa_topk_ratio", "QSA筛选比例", registry=self.registry
        )
        self.qsa_computation_time = Histogram(
            "quantum_arch_qsa_computation_time_seconds", "QSA计算时间", registry=self.registry
        )
        self.qsa_interference_score = Gauge(
            "quantum_arch_qsa_interference_score", "QSA干涉评分", registry=self.registry
        )

        # 3. QEL指标
        self.qel_entanglement_strength = Gauge(
            "quantum_arch_qel_entanglement_strength", "QEL纠缠强度", registry=self.registry
        )
        self.qel_qft_time = Histogram(
            "quantum_arch_qel_qft_time_seconds", "QEL QFT计算时间", registry=self.registry
        )

        # 4. QCI指标
        self.qci_early_exit_rate = Gauge(
            "quantum_arch_qci_early_exit_rate", "QCI早退率", registry=self.registry
        )
        self.qci_avg_entropy = Gauge(
            "quantum_arch_qci_avg_entropy", "QCI平均熵", registry=self.registry
        )
        self.qci_tau_low = Gauge(
            "quantum_arch_qci_tau_low", "QCI早退低阈值", registry=self.registry
        )
        self.qci_tau_high = Gauge(
            "quantum_arch_qci_tau_high", "QCI早退高阈值", registry=self.registry
        )

        # 5. QGD指标
        self.qgd_mod_lr = Gauge("quantum_arch_qgd_mod_lr", "QGD模长学习率", registry=self.registry)
        self.qgd_phase_lr = Gauge(
            "quantum_arch_qgd_phase_lr", "QGD相位学习率", registry=self.registry
        )
        self.qgd_mod_grad_norm = Gauge(
            "quantum_arch_qgd_mod_grad_norm", "QGD模长梯度范数", registry=self.registry
        )
        self.qgd_phase_grad_norm = Gauge(
            "quantum_arch_qgd_phase_grad_norm", "QGD相位梯度范数", registry=self.registry
        )

        # 6. 酉约束指标
        self.unitarity_violation = Gauge(
            "quantum_arch_unitarity_violation", "酉约束违背度", registry=self.registry
        )
        self.matrix_condition_number = Gauge(
            "quantum_arch_matrix_condition_number", "矩阵条件数", registry=self.registry
        )

        # 7. 性能指标
        self.batch_processing_time = Histogram(
            "quantum_arch_batch_processing_time_seconds", "批处理时间", registry=self.registry
        )
        self.step_time = Gauge("quantum_arch_step_time_seconds", "每步时间", registry=self.registry)
        ...
        self.gpu_utilization = Gauge(
            "quantum_arch_gpu_utilization", "GPU利用率", registry=self.registry
        )
        self.gpu_memory_used = Gauge(
            "quantum_arch_gpu_memory_used_gb", "GPU显存使用（GB）", registry=self.registry
        )
        self.gpu_memory_allocated = Gauge(
            "quantum_arch_gpu_memory_allocated_gb", "GPU显存分配（GB）", registry=self.registry
        )

        # 9. 成本指标
        self.daily_cost = Gauge(
            "quantum_arch_daily_cost_usd", "每日成本（美元）", registry=self.registry
        )
        self.hourly_cost = Gauge(
            "quantum_arch_hourly_cost_usd", "每小时成本（美元）", registry=self.registry
        )

        # 10. 系统指标
        self.active_steps = Counter(
            "quantum_arch_active_steps_total", "总训练步数", registry=self.registry
        )
        self.checkpoints_saved = Counter(
            "quantum_arch_checkpoints_saved_total", "保存的checkpoint数量", registry=self.registry
        )
