...
class TestComplexOps:
    ...
    def test_born_normalize(self, device):
        ...
        # 验证概率和为1
        probs = normalized.abs() ** 2
        prob_sum = probs.sum(dim=-1)
        torch.testing.assert_close(prob_sum, torch.ones_like(prob_sum), atol=1e-5)
...
class TestQuantumEntanglementLayer:
    ...
    def test_qel_entanglement_strength(self, complex_input, device):
        ...
        # 记录原始纠缠度量
        output1, _ = qel(complex_input)
        
        # 修改纠缠强度
        qel.entanglement_strength = 0.5
        output2, _ = qel(complex_input)
...
class TestPositionalEncoding:
    ...
    def test_quantum_positional_encoding(self, complex_input, device):
        """测试量子位置编码"""
        pe = QuantumPositionalEncoding(dim=64, max_seq_len=2048).to(device)
    ...
    def test_learned_positional_encoding(self, complex_input, device):
        ...
        pe = LearnedPositionalEncoding(dim=64, max_seq_len=2048, dropout=0.1).to(device)
...
class TestQGD:
    ...
    def test_qgd_initialization(self, device):
        ...
        optimizer = QGD(model.parameters(), lr=0.001)
    ...
    def test_qgd_training_step(self, device):
        """测试训练步骤"""
        model = QuantumArch(dim=32, num_layers=1).to(device)
        optimizer = QGD(model.parameters(), lr=0.01)
