...
def test_von_neumann_entropy(device):
    ...
    # 优化版本
    time_opt = benchmark_function(
        von_neumann_entropy_optimized, probs, dim=-1, warmup=10, num_runs=100
    )
...
def test_fused_operators(device):
    ...
    time_fused = benchmark_function(
        fused_qkv_attention, Q, K, V, scale, 0.1, True, warmup=10, num_runs=50
    )
...
def test_torch_compile(device):
    ...
    if hasattr(torch, "compile"):
        try:
            ...
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            time_compile = benchmark_function(
                qsa_compiled, x, training=False, warmup=3, num_runs=10
            )
            ...
            print(f"  加速比: {speedup:.2f}x")
            return {
                "no_compile_ms": time_no_compile,
                "compile_ms": time_compile,
                "speedup": speedup,
            }
